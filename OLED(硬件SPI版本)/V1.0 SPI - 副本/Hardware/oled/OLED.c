...
void OLED_W_RES(uint8_t BitValue)
{
	...
	GPIO_WriteBit(RES_PORT, RES_PIN, (BitAction)BitValue);
}
...
void OLED_W_DC(uint8_t BitValue)
{
	...
	GPIO_WriteBit(DC_PORT, DC_PIN, (BitAction)BitValue);
}
...
void OLED_W_CS(uint8_t BitValue)
{
	...
	GPIO_WriteBit(CS_PORT, CS_PIN, (BitAction)BitValue);
}
...
void OLED_GPIO_Init(void)
{
    ...
    // 复用推挽输出
    GPIO_InitStructure.GPIO_Pin = D0_SCK_PIN | D1_MOSI_PIN;
	...
	GPIO_ResetBits(D0_SCK_PORT, D0_SCK_PIN);
	GPIO_SetBits(D1_MOSI_PORT,D1_MOSI_PIN);
	OLED_W_RES(1);
	OLED_W_DC(1);
	OLED_W_CS(1);
}
...
/**
  ...
  */
void OLED_SetCursor(int16_t Page, int16_t X)
{
...
/**
  ...
  */
uint8_t OLED_pnpoly(int16_t nvert, int16_t *vertx, int16_t *verty, int16_t testx, int16_t testy)
{
	int16_t i, j, c = 0;
...
void OLED_Update(void)
{
	...
	OLED_Printf(128-7*6, 0, 6,"FPS %d", Get_FPS());
...
void OLED_Printf(uint8_t X, uint8_t Y, uint8_t FontSize, char *format, ...)
{
	...
	va_end(arg);							//结束变量arg
	Menu_ShowString(X, Y, String, FontSize);//OLED显示字符数组（字符串）
	Menu_strlen(String);
}
...
void OLED_DrawArc(uint8_t X, uint8_t Y, uint8_t Radius, int16_t StartAngle, int16_t EndAngle, uint8_t IsFilled)
{
	...
	/*在画圆的每个点时，判断指定点是否在指定角度内则画点，不在，则不做处理*/
